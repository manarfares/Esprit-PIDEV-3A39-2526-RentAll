...
public class ToolController {
    ...
    // ── Current user (injected by MainController) ──────────────────────────────
     ...
     * Applies role-based UI restrictions and reloads data for the correct scope.
     */
        ...
        switch (role) {
            case "ROLE_ADMIN" -> {
                ...
                tableView.getSelectionModel().getSelectedItems().addListener(
                    (javafx.collections.ListChangeListener<Tool>) c -> {
            ...
            case "ROLE_HOST" -> {
                addQualityColumn();
                rbAll.setVisible(false);    rbAll.setManaged(false);
                rbPending.setVisible(true); rbPending.setManaged(true);
                rbActiveOnly.setVisible(true);
            }
            default -> {
                // Guest / ROLE_HOST_PENDING — read-only
                ...
                rbAll.setVisible(false);        rbAll.setManaged(false);
                rbPending.setVisible(false);    rbPending.setManaged(false);
                rbActiveOnly.setVisible(false); rbActiveOnly.setManaged(false);
            }
        }
    }
    ...
    // ── Data loading (role-scoped) ─────────────────────────────────────────────
    private void loadData() {
        Task<List<Tool>> task = new Task<>() {
            @Override protected List<Tool> call() throws Exception {
                if (currentUser == null) return dao.findAll();
                return switch (currentUser.getRole()) {
                    case "ROLE_ADMIN" -> dao.findAll();
                    case "ROLE_HOST"  -> dao.findByHostId(currentUser.getId());
                    default           -> dao.findAllActive();
                };
            }
        };
    ...
    private void openEditDialog(Tool tool) {
        if (currentUser != null && currentUser.getRole().equals("ROLE_HOST")) {
            if (tool.getHostId() != currentUser.getId()) {
                Alerts.showError("Access Denied", "You can only edit your own tools.");
                return;
            }
        }
    ...
    @FXML private void onBulkApprove() {
        ...
        if (ids.isEmpty()) { Alerts.showError("No Selection", "Select at least one tool."); return; }
        try { dao.setActiveAll(ids, true); loadData(); }
    ...
    @FXML private void onBulkDelete() {
        ...
        boolean confirmed = Alerts.showConfirmation("Bulk Delete",
            "Delete " + ids.size() + " tool(s)? This cannot be undone.");
        if (!confirmed) return;
    ...
    @FXML private void onExportCsv() {
        ...
        chooser.setTitle("Export Tools as CSV");
        chooser.setInitialFileName("tools_export.csv");
        ...
        try (FileWriter fw = new FileWriter(file)) {
            fw.write("ID,Name,Description,Price/Day,Stock,Location,Status,Host ID,Category ID\n");
            for (Tool t : sortedList) {
                fw.write(String.join(",",
                    String.valueOf(t.getId()),
                    csvEscape(t.getName()),
                    csvEscape(t.getDescription()),
                    t.getPricePerDay() != null ? t.getPricePerDay().toPlainString() : "",
                    String.valueOf(t.getStockQuantity()),
                    csvEscape(t.getLocation()),
                    t.isActive() ? "Active" : "Pending",
                    String.valueOf(t.getHostId()),
                    t.getCategoryId() != null ? String.valueOf(t.getCategoryId()) : ""
                ) + "\n");
            }
            lblStatus.setText("✅ Exported " + sortedList.size() + " tools to " + file.getName());
        } catch (IOException e) {
    ...
    private List<Integer> getSelectedIds() {
        return tableView.getSelectionModel().getSelectedItems()
            .stream().map(Tool::getId).collect(Collectors.toList());
    }
    ...
    private void refreshPendingBanner() {
        ...
        task.setOnSucceeded(e -> {
            ...
            if (count > 0) {
                lblPendingCount.setText(count + " tool" + (count > 1 ? "s" : "") + " pending");
    ...
    @FXML private void onAdd() {
        ...
        result.ifPresent(tool -> {
            try {
                tool.setHostId(currentUser != null ? currentUser.getId() : dao.resolveDefaultHostId());
                boolean autoApprove = currentUser != null && currentUser.getRole().equals("ROLE_ADMIN");
                dao.insert(tool, autoApprove);
                loadData();
    ...
    @FXML private void onDelete() {
        ...
        if (selected == null) return;
        if (currentUser != null && currentUser.getRole().equals("ROLE_HOST")) {
            if (selected.getHostId() != currentUser.getId()) {
                Alerts.showError("Access Denied", "You can only delete your own tools.");
                return;
            }
        }
