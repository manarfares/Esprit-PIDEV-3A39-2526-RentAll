...
public class ServiceController {
    ...
    private void applyRoleRestrictions() {
        ...
        switch (role) {
            ...
            case "ROLE_HOST" -> {
                ...
                addQualityColumn();
                // Hide the approval filter bar (hosts only see their own items)
            ...
            default -> {
                ...
                // Hide approval filter (guests only see active items anyway)
                rbAll.setVisible(false);        rbAll.setManaged(false);
    ...
    // ── Dynamic columns (added after role check) ───────────────────────────────
    ...
    private void updatePredicate() {
        ...
        filteredList.setPredicate(service -> {
            ...
            // 2. Approval toggle (only relevant for admin/host views)
    ...
    private void openEditDialog(Service service) {
        ...
        if (currentUser != null && currentUser.getRole().equals("ROLE_HOST")) {
            if (service.getHostId() != currentUser.getId()) {
                Alerts.showError("Access Denied", "You can only edit your own services.");
                return;
            }
        }
    ...
    @FXML private void onDelete() {
        ...
        if (currentUser != null && currentUser.getRole().equals("ROLE_HOST")) {
            if (selected.getHostId() != currentUser.getId()) {
                Alerts.showError("Access Denied", "You can only delete your own services.");
                return;
            }
        }
